...
export default function(options) {
  ...
  (options.protocol === 'http') ? options.config.agent = httpAgent : options.config.agent = httpsAgent;
  ...
  const request = new Promise((resolve, reject) => {
    const wrappedRequest = (n) => {
      req = protocol.request(options.config, (response) => {
        let body = '';
        response.on('data', (d)  => {
          body += d;
        });
      ...
      req.on('abort', (err) => {
        if (callback) callback(Constants.REQUEST_ABORTED, null);
        reject(Constants.REQUEST_ABORTED);
      })
      req.on('error', (err) => {
        if (n > 0) {
          retry();
  ...
  request.abort = () => {
    if(req.abort && (typeof req.abort === 'function')) {
      return req.abort(); // deprecated since node v.13.14.0
    }
    return req.destroy();
  }
  return request;
